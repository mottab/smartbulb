...
module.exports = function(model) {
    ...
    router.get('/', function list(req, res) {
      if(req.headers['x-bulb-api-key'] === config.API_KEY.hash){
        res.status(200).json({
              message: 'Welcom to ibtikar smart bulb rooms backend system, ' + 
    ...
    router.post('/', function insert(req, res) {
  	...
  	router.get('/:room(\\d+)', function get(req, res) {
      if(req.headers['x-bulb-api-key'] === config.API_KEY.hash){
  	...
  	router.put('/:room(\\d+)', function update(req, res) {
      if(req.headers['x-bulb-api-key'] === config.API_KEY.hash){
    ...
    router.delete('/:room(\\d+)', function _delete(req, res) {
      if(req.headers['x-bulb-api-key'] === config.API_KEY.hash){
